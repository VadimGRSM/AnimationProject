...
from .models import Frame, FrameLock, Layer, LayerLock, ProjectMember, ProjectPresenceSession
from .presence import get_presence_cutoff

FRAME_LOCK_TTL_SECONDS = 35
LAYER_LOCK_TTL_SECONDS = FRAME_LOCK_TTL_SECONDS
...
def get_layer_lock_expires_at(now=None):
    now = now or timezone.now()
    return now + timedelta(seconds=LAYER_LOCK_TTL_SECONDS)


def _serialize_layer_lock(lock):
    return {
        'frame_id': lock.frame_id,
        'frame_index': lock.frame.index,
        'layer_id': lock.layer_id,
        'layer_name': lock.layer.name,
        'user_id': lock.user_id,
        'display_name': getattr(lock.user, 'display_name', '') or getattr(lock.user, 'email', '') or f'User {lock.user_id}',
        'email': getattr(lock.user, 'email', ''),
        'role': getattr(lock.presence_session, 'role', '') or '',
...
def _get_layer_lock_for_serialization(lock_pk):
    return LayerLock.objects.select_related('frame', 'layer', 'user', 'presence_session').get(pk=lock_pk)


def _active_layer_lock_queryset(project_id, now=None):
    now = now or timezone.now()
    presence_cutoff = get_presence_cutoff(now)
    return LayerLock.objects.filter(
        ...
        'user',
        'presence_session',
    ).order_by('frame__index', 'frame_id', 'layer__order', 'layer_id')


def cleanup_stale_layer_locks(project_id, now=None):
    now = now or timezone.now()
    presence_cutoff = get_presence_cutoff(now)
    stale_locks = list(
        LayerLock.objects.filter(project_id=project_id)
        .select_related('frame', 'layer', 'user', 'presence_session')
    ...
    unique_stale_locks = {lock.pk: lock for lock in stale_locks if lock.pk is not None}
    if unique_stale_locks:
        LayerLock.objects.filter(pk__in=list(unique_stale_locks.keys())).delete()
        logger.info(
            "Stale layer locks cleaned up",
            extra={"project_id": project_id, "stale_lock_count": len(unique_stale_locks)},
        )
    return [_serialize_layer_lock(lock) for lock in unique_stale_locks.values()]


def get_project_layer_lock_snapshot(project_id, now=None):
    now = now or timezone.now()
    stale_releases = cleanup_stale_layer_locks(project_id, now=now)
    locks = [_serialize_layer_lock(lock) for lock in _active_layer_lock_queryset(project_id, now=now)]
    return {
        'locks': locks,
        'stale_releases': stale_releases,
    }
...
def _user_can_lock_layer(role):
    return role in {ProjectMember.Role.OWNER, ProjectMember.Role.EDITOR}


@transaction.atomic
def acquire_layer_lock(project_id, frame_id, layer_id, user_id, role, presence_session_id):
    now = timezone.now()
    stale_releases = cleanup_stale_layer_locks(project_id, now=now)
    ...
    if not _user_can_lock_layer(role):
        return {
            'status': 'denied',
            'reason': 'read_only_role',
            'lock': None,
    ...
    if (
        ...
        return {
            'status': 'denied',
            'reason': 'locked_by_other',
            'lock': _serialize_layer_lock(_get_layer_lock_for_serialization(existing_lock.pk)),
            'released': stale_releases,
        }

    expires_at = get_layer_lock_expires_at(now)
    if existing_lock is None:
        lock = LayerLock.objects.create(
            project_id=project_id,
        ...
        existing_lock.frame_id = layer.frame_id
        existing_lock.user_id = user_id
        existing_lock.presence_session = presence_session
        existing_lock.last_heartbeat_at = now
        existing_lock.expires_at = expires_at
...
@transaction.atomic
def release_layer_locks(project_id, user_id, presence_session_id, layer_id=None):
    now = timezone.now()
    released = cleanup_stale_layer_locks(project_id, now=now)

    locks_qs = LayerLock.objects.select_related('frame', 'layer', 'user', 'presence_session').filter(
        project_id=project_id,
        user_id=user_id,
        presence_session_id=presence_session_id,
    )
    if layer_id is not None:
        locks_qs = locks_qs.filter(layer_id=layer_id)

    locks = list(locks_qs)
    released.extend(_serialize_layer_lock(lock) for lock in locks)
    if locks:
        LayerLock.objects.filter(pk__in=[lock.pk for lock in locks]).delete()
    return released


@transaction.atomic
def heartbeat_layer_lock(project_id, layer_id, user_id, presence_session_id):
    now = timezone.now()
    released = cleanup_stale_layer_locks(project_id, now=now)

    updated = LayerLock.objects.filter(
        project_id=project_id,
        layer_id=layer_id,
        user_id=user_id,
        presence_session_id=presence_session_id,
...
def presence_session_holds_layer_lock(project_id, layer_id, user_id, presence_session_id, now=None):
    now = now or timezone.now()
    presence_cutoff = get_presence_cutoff(now)
    return LayerLock.objects.filter(
        project_id=project_id,
